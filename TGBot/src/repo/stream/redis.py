...
class RedisConnector():
    ...
    async def _ensure_group_exists(self):
        try:
            ...
            if "BUSYGROUP" not in str(e):
                logger.error(f"🔴 Error creating Redis stream group: {e}", exc_info=True)
    
    async def proccess_read_loop(self, handle: Callable[[list[dict]], Awaitable[bool]], count: int = 1):
        ...
        async def _handle(data_raw: str | None | dict, msg_id: str):
            try:
                if not data_raw:
                    ...
                    return
                data_raw_ = data_raw if isinstance(data_raw, str) else list(data_raw.values())[0]
    ...
    async def _read_loop(self, handle: Callable[[list[dict]], Awaitable[bool]], count: int = 1):
    ...
    async def start_read(self, handle: Callable[[dict], Awaitable[bool]], count: int = 1):
